...
####amminia NH3-N ----###
# make a simple plot just of the ISU data
ammonia.plot <- mahadi.df |>
  # filter(farm == "ISU") |>
  ggplot(aes(x = crop, y = porewater_nh3_mgl_log10, 
             fill=site_type, color=site_type)) +
  stat_summary(
    fun = mean, geom = "bar",
    linewidth = .4,
  ...
  labs(
    x = "",
    y = expression(bold("Ammonia (log10+1 mg L"^-1*")"))
  ) +
  scale_x_discrete(labels = c(
    "PCRO" = "Pea, Clover,\nRadish, Oat", 
    "CR" = "Cereal\nRye", 
  ...
  # theme_classic()+
  geom_hline(yintercept = c(0,0.05,0.10, 0.15), 
                            linetype = "dashed", color = "black", alpha=0.2)+
  theme_poster()+
  labs(title = "C) Ammonia")+
  theme(plot.title = element_text(hjust = 0.5, size = 24))  # Center the title

ammonia.plot


ggsave(ammonia.plot, file = "figures/nh3_poster_f.png", width = 15, height = 6, units="in")
