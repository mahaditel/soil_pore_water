...
bm.plot <- mahadi_biomass.df |>
  # filter(farm == "ISU") |>
  ggplot(aes(x = crop, y = net_biomass, color = farm, fill = farm)) + 
  ...
  geom_hline(yintercept = c(0, 100, 200, 300), linetype = "dashed", color = "black", alpha = 0.5) +  # Add dashed horizontal lines
...
ggsave(bm.plot, file="figures/biomass_f.png", units = "in", 
       width = 15, height = 6)
